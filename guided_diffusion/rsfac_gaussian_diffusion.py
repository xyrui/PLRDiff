...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.dpm_ddim_var_co = self.sqrt_one_minus_alphas_cumprod / np.sqrt(alphas) - self.sqrt_one_minus_alphas_cumprod_prev
    ...
    def sample_loop(
        ...
        for (sample, E) in self.sample_loop_progressive(
            model,
            shape,
            Rr,
            noise=noise,
    ...
    def sample_loop_progressive(self,
        ...
        if noise is not None:
            noise = th.index_select(noise.to(device), 1, Eband)
            img = self.q_sample(noise, th.tensor((start_t-1) * shape[0], device=device))
            del noise
        else:
        ...
        if param['degrade'] == 0:
            pd = int((param['k_s'] - 1)/2)
            blur = Compose([partial(nF.pad, pad = [pd, pd, pd, pd], mode='circular'), partial(nF.conv2d, weight=param['kernel'], padding=0, groups=Cc)])
            down = partial(nF.interpolate, scale_factor=1/param['scale'], mode='nearest')
        elif param['degrade'] == 1:
            blur = partial(nF.conv2d, weight=param['kernel'], padding=int((param['k_s'] - 1)/2), groups=Cc)
            down = partial(imresize, scale=1/param['scale'])
        elif param['degrade'] == 3:
            pd = int((param['k_s'] - 1)/2)
            blur = partial(nF.conv2d, weight=param['kernel'], padding=int((param['k_s'] - 1)/2), groups=Cc)
            down = partial(nF.interpolate, scale_factor=1/param['scale'], mode='nearest')

        # estimate coefficient matrix E   
        bimg = th.index_select(LRMS, 1, Eband).reshape(Bb, Rr, -1).cpu() # base tensor from LRMS
        ...
        t2 = th.matmul(LRMS.reshape(Bb, Cc, -1).cpu(), bimg.transpose(1,2))
    ...
    def ddim_sample(
        ...
        sample = ( _extract_into_tensor(self.sqrt_alphas_cumprod_prev, t, x.shape)*pred_xstart
                  + _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod_prev, t, x.shape)*model_output
                  )
